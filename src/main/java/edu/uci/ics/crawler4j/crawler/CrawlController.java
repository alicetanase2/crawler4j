...
public class CrawlController extends Configurable {
    ...
    public <T extends WebCrawler> void start(final Class<T> _c, final int numberOfCrawlers) {
        this.start(_c, numberOfCrawlers, true, new StringBuffer());
    }
    ...
    public <T extends WebCrawler> void startNonBlocking(final Class<T> _c, final int numberOfCrawlers) {
        this.start(_c, numberOfCrawlers, false, new StringBuffer());
    }

    protected <T extends WebCrawler> void start(final Class<T> _c, final int numberOfCrawlers, boolean isBlocking, StringBuffer stringBuffer) {
        try {
            finished = false;
                            ...
                            while (true) {
                                ...
                                if (!someoneIsWorking) {
                                    ...
                                    if (!someoneIsWorking) {
                                        ...
                                        File mergedFile = new File("/Users/alice/Documents/WebCrawler/crawl4jFinalFile/newFile.txt");
                                        
                                        File[] finalFiles = new File[files.length-1];
                                        for (int i = 1; i < files.length; i++){
                                            finalFiles[i-1] = files[i];
                                        }
                                        
                                        mergeFiles(finalFiles, mergedFile);
                                        return;
                                    }
                                }
                            }
